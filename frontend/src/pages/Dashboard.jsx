      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">{greeting()}, {user?.fullName || user?.username}</h1>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-3">Open pipeline</p>
                <p className="text-3xl font-bold text-gray-900">{fmt(pipelineValue)}</p>
                <p className="text-xs text-gray-400 mt-1.5">SENT + PENDING offertes</p>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-3">Accepted this month</p>
                <p className="text-3xl font-bold text-gray-900">{acceptedThisMonth.length}</p>
                <p className="text-xs text-gray-400 mt-1.5">offerte{acceptedThisMonth.length !== 1 ? 's' : ''} won</p>
              </div>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-3">Active projects</p>
                <p className="text-3xl font-bold text-gray-900">{projects.length}</p>
                <p className="text-xs text-amber-600 mt-1.5 font-semibold">View all →</p>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-gray-700">Recent offertes</p>
              </div>
              <button
            ...
            {recentOffertes.length === 0 ? (
              <div className="text-center py-14 text-gray-400 text-sm">No offertes yet</div>
            ) : (
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-gray-700">Upcoming tasks</p>
              </div>
              <button
            ...
            {tasks.length === 0 ? (
              <div className="text-center py-14 text-gray-400 text-sm">No open tasks</div>
            ) : (
              <div className="divide-y divide-gray-100">
                {tasks.map(task => {
